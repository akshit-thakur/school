 ...
 * WORLD MAP AND INDIVIDUAL CONTINENT MAPS AND/OR BIG COUNTRY MAPS.(7-10-16) Can also make something like reinforcments instead of bonuses.
...
class Risk{
    ...
    String p1,p2;
    Risk(){
         dice1=0;  dice2=0; sum1=0; sum2=0;  sumC=0;  ter1=0; ter2=0;  terC=0; total=0; mark=0;  moves=0; turn=0; bonus1=0; bonus2=0;
         p1="";      p2="";
    }
    ...
    void starter(){
        try{
            ...
            switch(flag){
                case 0: System.out.println("Enter name,player 1:");
                p1=read.nextLine();    flag=1;
                case 1: System.out.println("Enter name,player 2:");
                p2=read.nextLine();    flag=2;
                case 2: System.out.println("Enter number of total territories: ");
                total=read.nextInt(); flag=3;
                case 3: System.out.println("Enter %age of territories to decide winner: ");
                mark=(int)(read.nextInt()*(total))/100; flag=4;                  
                rules();    playPercent(); /*next method called*/
            }
        }
    ...
    void rules(){
        System.out.println(p1+" has first attacking initiative.\n Attacker has to roll dices having sum greater than that of defender's");
        System.out.println("After each attack,the initiative changes to and fro.\n If sums are equal,the defender gets advantage.");
        System.out.println("The aim is to get "+mark+" number of territories(i.e. the specified %age) under your control in percent vicotry mode.");
    }
    /*void bonus(){
       if(total<21){
           bonus1=10-(ter1*total)/10;
           bonus2=10-(ter2*total)/10;
       }
       else if(total>20&&total<51){
           bonus1=8-(ter1*total)/20;
           bonus2=8-(ter2*total)/20;
       }
       else if(total>50&&total<101){
           bonus1=10-(ter1*total)/100;
           bonus2=10-(ter2*total)/100;
       }
       else{
           bonus1=20-(ter1*total)/100;
