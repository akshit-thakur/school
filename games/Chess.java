...
import java.util.*;
class Chess{
    String board[][]=new String[8][8];   String p1,p2; 
    int n,current,target,currentC,targetC,targetR,currentR; String piece;  
    Scanner read=new Scanner(System.in);
    Chess(){
        n=0; current=0; target=0; targetR=0; currentR=0; currentC=0;targetC=0; piece=""; p1=""; p2="";
    }
    void rules(){
    ...
    void fillBoard(){
        ...
        board[0][0]="r";        board[0][7]="r";        board[7][0]="R";    board[7][7]="R";
        board[0][1]="h";        board[0][6]="h";        board[7][1]="H";    board[7][6]="H";
        board[0][2]="b";        board[0][5]="b";        board[7][2]="B";    board[7][5]="B";
        board[0][3]="k";        board[0][4]="q";        board[7][3]="K";    board[7][4]="Q";
    }
    void dispBoard(){
        System.out.print("\t\t\t   ");
        for(int i=1;i<=8;i++)
    ...
    boolean pieceNotFound(String str){   
        System.out.println(currentR+","+currentC+":"+targetR+","+targetC+","+board[currentR][currentC]+","+board[targetR][targetC]+","+str);
    ...
    boolean moveValid(){
         ...
         * return king(r,currentC,targetR,targetC);
        else if(piece.equalsIgnoreCase("q"))
        return queen(r,currentC,targetR,targetC);
        else if(piece.equalsIgnoreCase("h"))
        return knight(r,currentC,targetR,targetC);
        else if(piece.equalsIgnoreCase("b"))
        return bishop(r,currentC,targetR,targetC);*/
        else
        return false;
    }
    boolean pawn(){ 
    ...
    void getInput(){        
        try{
            if((n%2)==0)
            System.out.println(p1+",Enter your desired piece.e.g. P for pawn.Type any number to quit.");
            else
            System.out.println(p2+",Enter your desired piece.e.g. p for pawn.Type any number to quit.");
            String piece=read.nextLine();
            if(Character.isDigit(piece.charAt(0))){
                ...
                target=read.nextInt();
                currentR=(current/10)-1;currentC=(current%10)-1;
                targetR=(target/10)-1;targetC=(target%10)-1;                 
    ...
    void starter(){
         rules();
         System.out.println("Are you ready? Press y to continue or any other key to exit");
         char go=read.next().charAt(0);
         if(!(Character.toLowerCase(go)=='y')){
             System.out.println("See you soon");
             System.exit(0);
         }
         else{
             fillBoard();
             System.out.println("Enter name,player 1: "); p1=read.nextLine(); read.nextLine(); 
             System.out.println("Enter name,player 2: "); p2=read.nextLine();
             dispBoard();
             getInput();
         }
    }
    public static void main(String args[]){
        Chess call=new Chess();
        call.starter();        
    }
}